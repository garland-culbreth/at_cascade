...
def main() :
   ...
   # check that random effects sum to zero
   var_csv = at_cascade.csv.read_table(f'{fit_dir}/n0/variable.csv')
   re_sum = 0
   for row in range(len(var_csv)):
      if (var_csv[row]['fixed'] == 'true' and var_csv[row]['rate'] != 'omega'):
         re_sum += float(var_csv[row]['fit_value'])
   print(re_sum)
   assert re_sum < 1e-10, 'random effects sum too large'
   # 
